...
function validateNormalizedOptions(options) {
    validateOptions(options);
    validateOutputConfig(options.outputConfig);
    validateStylesheetConfig(options.stylesheetConfig);
}
exports.validateNormalizedOptions = validateNormalizedOptions;
function validateOptions(options) {
    errors_1.invariant(!utils_1.isUndefined(options), errors_1.CompilerValidationErrors.MISSING_OPTIONS_OBJECT, [options]);
    errors_1.invariant(utils_1.isString(options.name), errors_1.CompilerValidationErrors.INVALID_NAME_PROPERTY, [options.name]);
    errors_1.invariant(utils_1.isString(options.namespace), errors_1.CompilerValidationErrors.INVALID_NAMESPACE_PROPERTY, [options.namespace]);
...
function validateStylesheetConfig(config) {
    ...
    if (!utils_1.isUndefined(customProperties)) {
        ...
        if (!utils_1.isUndefined(resolution)) {
            errors_1.invariant(utils_1.isObject(resolution), errors_1.CompilerValidationErrors.INVALID_RESOLUTION_PROPERTY, [resolution]);
...
function validateOutputConfig(config) {
    ...
    if (!utils_1.isUndefined(config.env)) {
        errors_1.invariant(utils_1.isObject(config.env), errors_1.CompilerValidationErrors.INVALID_ENV_PROPERTY, [config.env]);
        for (const [key, value] of Object.entries(config.env)) {
            errors_1.invariant(KNOWN_ENV.has(key), errors_1.CompilerValidationErrors.UNKNOWN_ENV_ENTRY_KEY, [key]);
            errors_1.invariant(utils_1.isString(value), errors_1.CompilerValidationErrors.INVALID_ENV_ENTRY_VALUE, [key, value]);
        }
    }
}
function normalizeOptions(options) {
    const outputConfig = Object.assign({}, DEFAULT_OUTPUT_CONFIG, options.outputConfig);
    const stylesheetConfig = {
        customProperties: Object.assign({}, DEFAULT_STYLESHEET_CONFIG.customProperties, (options.stylesheetConfig && options.stylesheetConfig.customProperties))
    };
    return Object.assign({}, DEFAULT_OPTIONS, options, { stylesheetConfig,
        outputConfig });
}
