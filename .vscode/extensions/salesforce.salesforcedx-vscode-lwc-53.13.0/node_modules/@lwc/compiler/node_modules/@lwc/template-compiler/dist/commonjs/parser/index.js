...
function parse(source, state) {
    ...
    function getTemplateRoot(documentFragment) {
        ...
        const validRoots = documentFragment.childNodes.filter((child) => (html_1.treeAdapter.isElementNode(child) ||
            html_1.treeAdapter.isTextNode(child) && html_1.treeAdapter.getTextNodeContent(child).trim().length));
    ...
    function applyLwcDirective(element) {
        const lwcDomAttribute = getTemplateAttribute(element, 'lwc:dom');
        if (!lwcDomAttribute) {
            return;
        }
        attribute_1.removeAttribute(element, lwcDomAttribute.name);
        ...
        if (lwcDomAttribute.type !== types_1.IRAttributeType.String || types_1.LWCDirectiveDomMode.hasOwnProperty(lwcDomAttribute.value) === false) {
            const possibleValues = Object.keys(types_1.LWCDirectiveDomMode).map((value) => `"${value}"`).join(', or ');
            return warnOnElement(errors_1.ParserDiagnostics.LWC_DOM_INVALID_VALUE, element.__original, [possibleValues]);
        }
        if (element.children.length > 0) {
            return warnOnElement(errors_1.ParserDiagnostics.LWC_DOM_INVALID_CONTENTS, element.__original);
        }
        element.lwc = {
            dom: lwcDomAttribute.value
        };
    }
    function applyLocator(element) {
        ...
        if (!locatorIdAttribute && locatorContextAttribute) {
            attribute_1.removeAttribute(element, locatorContextAttribute.name);
            return warnOnElement(errors_1.ParserDiagnostics.LOCATOR_CONTEXT_MUST_BE_USED_WITH_LOCATOR_ID, element.__original);
        }
        if (locatorIdAttribute) {
            attribute_1.removeAttribute(element, locatorIdAttribute.name);
            if (locatorContextAttribute) {
                attribute_1.removeAttribute(element, locatorContextAttribute.name);
            }
            if (locatorIdAttribute.type !== types_1.IRAttributeType.String) {
                return warnAt(errors_1.ParserDiagnostics.LOCATOR_ID_SHOULD_BE_STRING, [], locatorIdAttribute.location);
            }
            const id = locatorIdAttribute.value;
            let context;
            if (locatorContextAttribute !== undefined) {
                if (locatorContextAttribute.type !== types_1.IRAttributeType.Expression) {
                    return warnAt(errors_1.ParserDiagnostics.LOCATOR_CONTEXT_SHOULD_BE_EXPRESSION, [], locatorContextAttribute.location);
                }
                else {
                    context = locatorContextAttribute.value;
                    if (types_2.isMemberExpression(context)) {
                        return warnAt(errors_1.ParserDiagnostics.LOCATOR_CONTEXT_CANNOT_BE_MEMBER_EXPRESSION, [], locatorContextAttribute.location);
                    }
                }
            }
            element.locator = {
    ...
    function applyStylesheet(element, node) {
        if (!isStyleElement(element)) {
            return;
        }
        const src = node.childNodes.reduce((acc, n) => acc + n.value.trim(), '');
        if (src.length === 0) {
            warnOnElement(errors_1.ParserDiagnostics.EMPTY_STYLE_TAG, node);
        }
        if (state.config.format === 'function') {
            warnOnElement(errors_1.ParserDiagnostics.INVALID_STYLE_TAG_FUNCTION_FORMAT, node);
        }
        const isAtRoot = stack.length === 1;
        const isFirstChildElement = element.parent.children[0] === element;
        if (!isAtRoot || !isFirstChildElement) {
            warnOnElement(errors_1.ParserDiagnostics.INVALID_STYLE_TAG_POSITION, node);
        }
        try {
            element.inlineStyles = style_1.parseInlineStyle(src, state.config);
        }
        catch (error) {
            ...
            return warnOnElement(errors_1.TransformerErrors.CSS_IN_HTML_ERROR, node, [message]);
        }
        finally {
            // Clear all the nodes.
            node.childNodes = [];
    ...
    function applyComponent(element) {
        ...
        if (isAttr) {
            if (isAttr.type !== types_1.IRAttributeType.String) {
                return warnAt(errors_1.ParserDiagnostics.DEPRECATED_IS_ATTRIBUTE_CANNOT_BE_EXPRESSION, [], isAttr.location);
            }
    ...
    function applyAttributes(element) {
        ...
        attrsList.forEach((rawAttr) => {
            ...
            if (!ir_1.isCustomElement(element) && !attribute_1.isValidHTMLAttribute(element.tag, name)) {
                warnAt(errors_1.ParserDiagnostics.INVALID_HTML_ATTRIBUTE, [name, tag], location);
            }
            else if (ir_1.isCustomElement(element) && attribute_1.isProhibitedIsAttribute(name)) {
                warnAt(errors_1.ParserDiagnostics.IS_ATTRIBUTE_NOT_SUPPORTED, [name, tag], location);
            }
            ...
            if (attribute_1.isAttribute(element, name)) {
                ...
                const attrKey = attribute_1.isDeprecatedIsAttribute(name) ? 'is' : name;
                const node = element.__original;
                // record secure import dependency if xlink attr is detected
                if (name === 'xlink:href' && node.namespaceURI === constants_1.SVG_NAMESPACE_URI) {
    ...
    function validateElement(element) {
        ...
        if (tag === 'template') {
            ...
            const isNotAllowedHtmlTag = constants_1.HTML_TAG_BLACKLIST.has(tag);
            ...
            const isNotAllowedSvgTag = !constants_1.SVG_TAG_WHITELIST.has(tag);
            if (namespace === constants_1.SVG_NAMESPACE_URI && isNotAllowedSvgTag) {
                return warnOnElement(errors_1.ParserDiagnostics.FORBIDDEN_SVG_NAMESPACE_IN_TEMPLATE, node, [tag]);
            }
            const isNotAllowedMathMlTag = constants_1.MATHML_TAG_BLACKLIST.has(tag);
    ...
    function validateAttributes(element) {
        ...
        attrsList.forEach(attr => {
            ...
            if (node.namespaceURI === constants_1.HTML_NAMESPACE_URI &&
                tag === 'iframe' &&
                attrName === 'srcdoc') {
            ...
            for (const propName in props) {
                ...
                if (attribute_1.isTabIndexAttribute(attrName)) {
                    if (type !== types_1.IRAttributeType.Expression &&
                        ...
                        warnOnElement(errors_1.ParserDiagnostics.INVALID_TABINDEX_ATTRIBUTE, element.__original);
                    }
                }
            }
        }
    ...
    function collectMetadata(element) {
        if (ir_1.isCustomElement(element)) {
            state.extendedDependencies.push(metadata_1.getModuleMetadata(element));
        }
    }
