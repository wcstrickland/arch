...
function generateContext(element, data, codeGen) {
    ...
    // LWC
    if (lwc) {
        const lwcObject = Object.keys(lwc).map((key) => {
...
function transform(root, codeGen, state, options) {
    ...
    function transformElement(element, children) {
    ...
    function applyInlineForOf(element, babelNode) {
        ...
        const iterationFunction = t.functionExpression(undefined, functionParams, t.blockStatement([
    ...
    function computeAttrValue(attr, element) {
        ...
        switch (attr.type) {
            case types_1.IRAttributeType.Expression:
                let { expression } = scope_1.bindExpression(attr.value, element);
                ...
                if (attribute_1.isXLinkAttribute(attr.name) && namespaceURI === constants_2.SVG_NAMESPACE_URI) {
                    return t.callExpression(t.identifier('sanitizeAttribute'), [t.stringLiteral(tagName), t.stringLiteral(namespaceURI), t.stringLiteral(attr.name), expression]);
                }
                return expression;
            case types_1.IRAttributeType.String:
                ...
                if (attribute_1.isXLinkAttribute(attr.name) && namespaceURI === constants_2.SVG_NAMESPACE_URI) {
                    return t.callExpression(t.identifier('sanitizeAttribute'), [t.stringLiteral(tagName), t.stringLiteral(namespaceURI), t.stringLiteral(attr.name), t.stringLiteral(attr.value)]);
                }
...
function generateTemplateFunction(templateRoot, state, options) {
...
function default_1(templateRoot, state, options) {
