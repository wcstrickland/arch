...
function isEsTreeIdentifier(node) {
    return node.type === 'Identifier';
}
function isEsTreeMemberExpression(node) {
    return node.type === 'MemberExpression';
}
function validateExpression(node, element, state) {
    ...
    if (isEsTreeMemberExpression(node)) {
        errors_1.invariant(state.config.experimentalComputedMemberExpression || !node.computed, errors_1.ParserDiagnostics.COMPUTED_PROPERTY_ACCESS_NOT_ALLOWED);
        ...
        if (isEsTreeIdentifier(object) && isEsTreeIdentifier(property)) {
            errors_1.invariant(property.name !== ITERATOR_NEXT_KEY || !ir_1.isBoundToIterator(object.name, element), errors_1.ParserDiagnostics.MODIFYING_ITERATORS_NOT_ALLOWED);
        }
        else {
