...
class ForceSourcePullExecutor extends util_1.SfdxCommandletExecutor {
    ...
    build(data) {
        const builder = new cli_1.SfdxCommandBuilder()
            .withDescription(messages_1.nls.localize('force_source_pull_default_scratch_org_text'))
            .withArg('force:source:pull')
            .withLogName('force_source_pull_default_scratch_org');
        if (this.flag === '--forceoverwrite') {
            builder
                .withArg(this.flag)
                .withDescription(messages_1.nls.localize('force_source_pull_force_default_scratch_org_text'));
        }
