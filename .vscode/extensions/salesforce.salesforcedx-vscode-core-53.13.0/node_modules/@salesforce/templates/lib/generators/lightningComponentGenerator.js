...
class LightningComponentGenerator extends sfdxGenerator_1.SfdxGenerator {
    ...
    writing() {
        const { template, outputdir, componentname, apiversion, type, internal } = this.options;
        if (type === 'aura') {
        ...
        if (type === 'lwc') {
            ...
            this.fs.copyTpl(this.templatePath(`${template}.js`), this.destinationPath(path.join(outputdir, fileName, `${fileName}.js`)), { className });
            this.fs.copyTpl(this.templatePath(`${template}.html`), this.destinationPath(path.join(outputdir, fileName, `${fileName}.html`)), {});
            if (!internal) {
                const masterLabel = kit_1.camelCaseToTitleCase(componentname).replace(/</g, '&lt;').replace(/>/g, '&gt;');
                this.fs.copyTpl(this.templatePath(`${template}.js-meta.xml`), this.destinationPath(path.join(outputdir, fileName, `${fileName}.js-meta.xml`)), { apiVersion: apiversion, masterLabel });
            }
        }
    }
}
