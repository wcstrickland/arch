...
exports.ModelDeserializer = void 0;
const soql_parser_1 = require("@salesforce/soql-common/lib/soql-parser");
const Parser = require("@salesforce/soql-common/lib/soql-parser/generated/SoqlParser");
...
const Soql = require("../model/model");
const Impl = require("../model/impl");
const util_1 = require("../model/util");
const headerCommentsImpl_1 = require("../model/impl/headerCommentsImpl");
class ModelDeserializer {
    constructor(soqlSyntax) {
    ...
    deserialize() {
        ...
        const { headerComments, headerPaddedSoqlText } = soql_common_1.soqlComments.parseHeaderComments(this.soqlSyntax);
        ...
        if (parseTree) {
            ...
            if (query && headerComments) {
                query.headerComments = new headerCommentsImpl_1.HeaderCommentsImpl(headerComments);
            }
        }
...
class ErrorIdentifier {
        ...
        this.knownErrors = [
            {
                ...
                message: messages_1.Messages.error_empty,
                predicate: (error) => this.parseTree instanceof antlr4ts_1.ParserRuleContext && this.parseTree.start.type === antlr4ts_1.Token.EOF,
            },
            {
                type: Soql.ErrorType.NOSELECT,
                message: messages_1.Messages.error_noSelect,
                predicate: (error, context) => context instanceof Parser.SoqlSelectClauseContext &&
                    context.exception instanceof antlr4ts_2.InputMismatchException &&
                    !this.hasNonErrorChildren(context),
            },
            {
                type: Soql.ErrorType.NOSELECTIONS,
                message: messages_1.Messages.error_noSelections,
                predicate: (error, context) => context instanceof Parser.SoqlSelectClauseContext &&
                    context.exception instanceof antlr4ts_2.NoViableAltException &&
                    !this.hasNonErrorChildren(context),
            },
            {
                type: Soql.ErrorType.NOFROM,
                message: messages_1.Messages.error_noFrom,
                predicate: (error, context) => context instanceof Parser.SoqlFromClauseContext &&
                    context.exception instanceof antlr4ts_2.InputMismatchException &&
                    !this.hasNonErrorChildren(context),
            },
            {
                type: Soql.ErrorType.INCOMPLETEFROM,
                message: messages_1.Messages.error_incompleteFrom,
                predicate: (error, context) => context instanceof Parser.SoqlIdentifierContext &&
                    context.parent instanceof Parser.SoqlFromExprContext &&
                    context.exception instanceof antlr4ts_2.InputMismatchException,
            },
            {
                type: Soql.ErrorType.INCOMPLETELIMIT,
                message: messages_1.Messages.error_incompleteLimit,
                predicate: (error, context) => context instanceof Parser.SoqlIntegerValueContext &&
                    this.hasAncestorOfType(context, Parser.SoqlLimitClauseContext),
            },
            {
                type: Soql.ErrorType.EMPTYWHERE,
                message: messages_1.Messages.error_emptyWhere,
                predicate: (error, context) => context instanceof Parser.SoqlWhereExprsContext && context.childCount === 0,
            },
            {
                type: Soql.ErrorType.INCOMPLETENESTEDCONDITION,
                message: messages_1.Messages.error_incompleteNestedCondition,
                predicate: (error, context) => context instanceof tree_1.ErrorNode && context.parent instanceof Parser.NestedWhereExprContext,
            },
            {
                type: Soql.ErrorType.INCOMPLETEANDORCONDITION,
                message: messages_1.Messages.error_incompleteAndOrCondition,
                ...
                (context instanceof Parser.SoqlWhereExprContext &&
                    ...
                    !this.hasNonErrorChildren(context)) ||
                    // leading AND/OR
                    (context instanceof tree_1.ErrorNode &&
            ...
            },
            {
                ...
                message: messages_1.Messages.error_incompleteNotCondition,
                predicate: (error, context) => context instanceof Parser.SoqlWhereExprContext &&
                    context.parent instanceof Parser.SoqlWhereNotExprContext &&
                    !this.hasNonErrorChildren(context),
            },
            {
                type: Soql.ErrorType.UNRECOGNIZEDCOMPAREVALUE,
                message: messages_1.Messages.error_unrecognizedCompareValue,
                predicate: (error, context) => ((context instanceof Parser.SoqlLiteralValueContext &&
            ...
            },
            {
                ...
                message: messages_1.Messages.error_unrecognizedCompareOperator,
                predicate: (error, context) => context instanceof Parser.SoqlWhereExprContext &&
                    context.childCount >= 2 &&
                    context.getChild(1) instanceof tree_1.ErrorNode &&
                    context.getChild(1).symbol === error.getToken(),
            },
            {
                type: Soql.ErrorType.UNRECOGNIZEDCOMPAREFIELD,
                message: messages_1.Messages.error_unrecognizedCompareField,
                predicate: (error, context) => context instanceof Parser.SoqlWhereExprsContext &&
                    context.childCount >= 1 &&
                    context.getChild(0) instanceof tree_1.ErrorNode &&
                    context.getChild(0).symbol === error.getToken(),
            },
            {
                type: Soql.ErrorType.NOCOMPAREVALUE,
                message: messages_1.Messages.error_noCompareValue,
                predicate: (error, context) => (((context instanceof Parser.SoqlLiteralValueContext &&
                    context.parent instanceof Parser.SimpleWhereExprContext) ||
                    (context instanceof Parser.SoqlLikeValueContext && context.parent instanceof Parser.LikeWhereExprContext)) &&
            ...
            },
            {
                ...
                message: messages_1.Messages.error_noCompareOperator,
                predicate: (error, context) => context instanceof Parser.SoqlWhereExprContext &&
                    context.childCount === 1 &&
                    context.getChild(0) instanceof tree_1.ErrorNode &&
                    context.getChild(0).symbol !== error.getToken(),
            },
            {
                ...
                message: messages_1.Messages.error_incompleteMultiValueList,
                predicate: (error, context) => (context instanceof Parser.SoqlWhereExprContext &&
            ...
            // unexpectedEOF is an EOF catch-all, make sure it is tested last
            {
                ...
                message: messages_1.Messages.error_unexpectedEOF,
                predicate: (error, context) => { var _a; return ((_a = error.getToken()) === null || _a === void 0 ? void 0 : _a.type) === antlr4ts_1.Token.EOF; },
            },
        ];
    ...
    findExceptionsAndErrorNodes(context) {
        if (context instanceof antlr4ts_1.ParserRuleContext && context.exception) {
            this.nodesWithExceptionsAndErrorNodes.push(context);
        }
        if (context instanceof tree_1.ErrorNode) {
            this.nodesWithExceptionsAndErrorNodes.push(context);
        }
    ...
    matchErrorToContext(error) {
        var _a, _b;
        for (const node of this.nodesWithExceptionsAndErrorNodes) {
            if (node instanceof antlr4ts_1.ParserRuleContext && ((_a = node.exception) === null || _a === void 0 ? void 0 : _a.getOffendingToken()) === error.getToken()) {
                return node;
            }
            // ErrorNode matches if location matches (or if location is off by one character when the error is on <EOF>)
            if (node instanceof tree_1.ErrorNode &&
                ...
                (node.symbol.charPositionInLine === error.getCharacterPositionInLine() ||
                    (node.symbol.charPositionInLine === error.getCharacterPositionInLine() - 1 &&
                        ((_b = error.getToken()) === null || _b === void 0 ? void 0 : _b.type) === antlr4ts_1.Token.EOF))) {
                return node;
            }
        }
        return undefined;
    }
    hasNonErrorChildren(context) {
        if (context.childCount > 0) {
            for (let i = 0; i < context.childCount; i++) {
                const child = context.getChild(i);
                if (!(child instanceof tree_1.ErrorNode)) {
                    return true;
                }
            }
        }
    ...
    hasAncestorOfType(context, type) {
...
class QueryListener {
    ...
    enterSoqlFromExpr(ctx) {
        ...
        if (hasAsClause) {
            ...
            as =
                safeAS !== undefined
                    ? this.toUnmodeledSyntax(safeAS.symbol, idContexts[1].stop, Soql.REASON_UNMODELED_AS)
                    : this.toUnmodeledSyntax(idContexts[1].start, idContexts[1].stop, Soql.REASON_UNMODELED_AS);
        }
        const safeUSING = ctx.soqlUsingClause();
        const using = safeUSING
            ? this.toUnmodeledSyntax(safeUSING.start, safeUSING.stop, Soql.REASON_UNMODELED_USING)
            : undefined;
        this.from = new Impl.FromImpl(sobjectName, as, using);
    }
    // @Override
    enterSoqlFromExprs(ctx) {
        ...
        exprContexts.forEach((exprContext) => {
            ...
            if (exprContext instanceof Parser.SoqlSelectColumnExprContext) {
                ...
                if (field instanceof Impl.UnmodeledSyntaxImpl) {
                    this.selectExpressions.push(this.toUnmodeledSyntax(exprContext.start, exprContext.stop, field.reason));
                }
                else {
                    let alias;
                    const aliasCtx = exprContext.soqlAlias();
                    if (aliasCtx) {
                        alias = this.toUnmodeledSyntax(aliasCtx.start, aliasCtx.stop, Soql.REASON_UNMODELED_ALIAS);
                    }
                ...
                // not a modeled case
                const reason = exprContext instanceof Parser.SoqlSelectInnerQueryExprContext
                ...
                this.selectExpressions.push(this.toUnmodeledSyntax(exprContext.start, exprContext.stop, reason));
            }
        });
    }
    enterSoqlLimitClause(ctx) {
        let value;
    ...
    enterSoqlWhereClauseMethod(ctx) {
        let condition = this.exprsToCondition(ctx.soqlWhereExprs());
        if (!util_1.SoqlModelUtils.isSimpleGroup(condition)) {
            condition = this.toUnmodeledSyntax(ctx.soqlWhereExprs().start, ctx.soqlWhereExprs().stop, Soql.REASON_UNMODELED_COMPLEXGROUP);
        }
    ...
    enterSoqlInnerQuery(ctx) {
        ...
        if (withCtx) {
            this.with = this.toUnmodeledSyntax(withCtx.start, withCtx.stop, Soql.REASON_UNMODELED_WITH);
        }
        const groupByCtx = ctx.soqlGroupByClause();
        if (groupByCtx) {
            this.groupBy = this.toUnmodeledSyntax(groupByCtx.start, groupByCtx.stop, Soql.REASON_UNMODELED_GROUPBY);
        }
        ...
        if (offsetCtx) {
            this.offset = this.toUnmodeledSyntax(offsetCtx.start, offsetCtx.stop, Soql.REASON_UNMODELED_OFFSET);
        }
        const bindCtx = ctx.soqlBindClause();
        if (bindCtx) {
            this.bind = this.toUnmodeledSyntax(bindCtx.start, bindCtx.stop, Soql.REASON_UNMODELED_BIND);
        }
        const recordTrackingTypeCtx = ctx.soqlRecordTrackingType();
        if (recordTrackingTypeCtx) {
            this.recordTrackingType = this.toUnmodeledSyntax(recordTrackingTypeCtx.start, recordTrackingTypeCtx.stop, Soql.REASON_UNMODELED_RECORDTRACKING);
        }
        const updateCtx = ctx.soqlUpdateStatsClause();
        if (updateCtx) {
            this.update = this.toUnmodeledSyntax(updateCtx.start, updateCtx.stop, Soql.REASON_UNMODELED_UPDATE);
        }
    }
    enterSoqlQuery(ctx) {
        const innerCtx = ctx.soqlInnerQuery();
        innerCtx.enterRule(this);
        this.query = new Impl.QueryImpl(this.select, this.from, this.where, this.with, this.groupBy, this.orderBy, this.limit, this.offset, this.bind, this.recordTrackingType, this.update);
    }
    getQuery() {
        return this.query;
    }
    toUnmodeledSyntax(start, stop, reason) {
        if (!stop && start) {
        ...
        const text = start.inputStream.getText(Interval_1.Interval.of(start.startIndex, stop.stopIndex));
    ...
    toOrderByField(ctx) {
        ...
        if (ctx instanceof Parser.SoqlOrderByColumnExprContext) {
            ...
            result = this.toUnmodeledSyntax(ctx.start, ctx.stop, Soql.REASON_UNMODELED_DISTANCE);
        }
        return result;
    }
    toField(ctx) {
        ...
        if (isFunctionRef) {
            result = this.toUnmodeledSyntax(ctx.start, ctx.stop, Soql.REASON_UNMODELED_FUNCTIONREFERENCE);
        }
        else {
    ...
    toCompareValue(ctx) {
        if (ctx instanceof Parser.SoqlColonExprLiteralValueContext) {
            return this.toUnmodeledSyntax(ctx.start, ctx.stop, Soql.REASON_UNMODELED_COLONEXPRESSION);
        }
    ...
    exprToCondition(ctx) {
        ...
        return this.toUnmodeledSyntax(ctx.start, ctx.stop, reason);
    }
}
