...
exports.messages = {
    ...
    streaming_handshake_timeout_text: 'Streaming API のハンドシェイクがソケットのタイムアウトで失敗しました。接続を確認し、再実行してください。',
    hotswap_warn_text: 'Apex デバッガセッションの間に Apex クラスやトリガを修正することはできません。デバッグ完了後に変更を保存してください。',
    created_exception_breakpoint_text: '%s にブレークポイント例外を作成しました。',
    removed_exception_breakpoint_text: '%s のブレークポイント例外を削除しました。',
    idle_warn_text: '%s 分間アイドル状態です。デバッガセッションを終了させない場合は、%s 分以内に、コードを実行またはステップ実行するか、変数を検査してください。',
    idle_terminated_text: '%s 分間アイドル状態のため、デバッガセッションが終了されました。',
    invalid_isv_project_config: 'ISV デバッグ用のプロジェクト設定が無効か未完了です。設定の Apex デバッガのページに戻り、新しいパートナーデバッグセッションを開始し、再実行してください。'
};
