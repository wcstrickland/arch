...
class JUnitReporter {
    format(testResult) {
        ...
        output += `errors="0"  `;
        output += `time="${utils_1.msToSecond(summary.testExecutionTimeInMs)}">\n`;
    ...
    buildProperties(testResult) {
        ...
        Object.entries(testResult.summary).forEach(([key, value]) => {
            ...
            if (timeProperties.includes(key)) {
                value = `${utils_1.msToSecond(value)} s`;
    ...
    buildTestCases(tests) {
        ...
        for (const testCase of tests) {
            junitTests += `${tab}${tab}<testcase name="${testCase.methodName}" classname="${testCase.apexClass.fullName}" time="${utils_1.msToSecond(testCase.runTime)}">\n`;
