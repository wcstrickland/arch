...
class HeapDumpService {
    ...
    replaceFrameVariablesWithHeapDump(frame) {
        ...
        if (heapdump && heapdump.getOverlaySuccessResult()) {
            ...
            for (const outerExtent of heapdumpResult.HeapDump.extents) {
                ...
                if (!this.isPrimitiveType(outerExtent.typeName)) {
                    for (const innerExtent of outerExtent.extent) {
                        ...
                        const refContainer = new apexReplayDebug_1.ApexVariableContainer('', '', outerExtent.typeName, ref);
            ...
            for (const outerExtent of heapdumpResult.HeapDump.extents) {
                for (const innerExtent of outerExtent.extent) {
                    const symbolName = innerExtent.symbols && innerExtent.symbols.length > 0
                    ...
                    if (symbolName && frameInfo.locals.has(symbolName)) {
                        ...
                        if (refVar) {
                            if (refVar.type === constants_1.LC_APEX_PRIMITIVE_STRING) {
                                localVar.value = refVar.value;
                            }
                            else {
                                const updatedVariable = this.createVariableFromReference(symbolName, refVar, new Map(), new Array());
                                if (updatedVariable) {
                        ...
                        for (const innerExtent of outerExtent.extent) {
                            ...
                            if (innerExtent.symbols && innerExtent.symbols.length > 0) {
                                for (const symName of innerExtent.symbols) {
                                    if (symName && symName.startsWith(constants_1.EXTENT_TRIGGER_PREFIX)) {
                                        const refVar = this.logContext
                                            .getRefsMap()
                                            .get(innerExtent.address);
                                        if (refVar) {
                                            const updatedVariable = this.createVariableFromReference(symName, refVar, new Map(), new Array());
                                            if (updatedVariable) {
                                            ...
                                            frameInfo.globals.set(symName, new apexReplayDebug_1.ApexVariableContainer(symName, this.createStringFromExtentValue(innerExtent.value.value), outerExtent.typeName));
                                        }
                                    }
                                }
                            }
    ...
    // child references
    createStringRefsFromHeapdump(heapdump) {
        const refsMap = this.logContext.getRefsMap();
        for (const outerExtent of heapdump.HeapDump.extents) {
            if (outerExtent.typeName.toLowerCase() !== constants_1.LC_APEX_PRIMITIVE_STRING) {
                continue;
            }
            for (const innerExtent of outerExtent.extent) {
                ...
                if (!refsMap.has(innerExtent.address)) {
                    refsMap.set(innerExtent.address, new apexReplayDebug_1.ApexVariableContainer('', innerExtent.value.value, constants_1.APEX_PRIMITIVE_STRING, innerExtent.address));
                }
                const refVar = refsMap.get(innerExtent.address);
                refVar.type = constants_1.APEX_PRIMITIVE_STRING;
                refVar.value = `'${innerExtent.value.value}'`;
            }
        }
    }
    isAddress(value) {
        return (typeof value === 'string' && value.startsWith(constants_1.ADDRESS_PREFIX));
    }
    isTriggerExtent(outerExtent) {
        if ((outerExtent.typeName.toLowerCase() === constants_1.LC_APEX_PRIMITIVE_BOOLEAN ||
            this.isCollectionType(outerExtent.typeName)) &&
            (outerExtent.count > 0 &&
                outerExtent.extent[0].symbols !== null &&
                outerExtent.extent[0].symbols.length > 0 &&
                outerExtent.extent[0].symbols[0].startsWith(constants_1.EXTENT_TRIGGER_PREFIX))) {
            return true;
        }
        return false;
    }
    ...
    copyReferenceContainer(refContainer, varName, createVarRef = true) {
    ...
    //   1: '2'
    //   2: '3'
    createStringFromVarContainer(varContainer, visitedSet) {
        // If the varContainer isn't a reference or is a string references
        if (!varContainer.ref ||
            varContainer.type.toLowerCase() === constants_1.LC_APEX_PRIMITIVE_STRING) {
        ...
        try {
            ...
            const isListOrSet = varContainer.type.toLowerCase().startsWith('list<') ||
    ...
    isPrimitiveType(typeName) {
        ...
        if (lcTypeName === constants_1.LC_APEX_PRIMITIVE_BLOB ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_BOOLEAN ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_DATE ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_DATETIME ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_DECIMAL ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_DOUBLE ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_ID ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_INTEGER ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_LONG ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_STRING ||
            lcTypeName === constants_1.LC_APEX_PRIMITIVE_TIME) {
            return true;
        }
        return false;
    }
    isCollectionType(typeName) {
        return (typeName.toLocaleLowerCase().startsWith('map<') ||
    ...
    updateLeafReferenceContainer(refContainer, extentValue, collectionType) {
        ...
        if (this.isCollectionType(refContainer.type)) {
            ...
            if (extentValue.value.entry) {
                ...
                for (const extentValueEntry of extentValue.value.entry) {
                    ...
                    const keyValueName = `${constants_1.KEY_VALUE_PAIR_KEY}${entryNumber.toString()}_${constants_1.KEY_VALUE_PAIR_VALUE}${entryNumber.toString()}`;
                    const keyValueContainer = new apexReplayDebug_1.ApexVariableContainer(keyValueName, '', constants_1.KEY_VALUE_PAIR);
                    // create the key variable, ensure the key's type is set
                    const keyContainer = new apexReplayDebug_1.ApexVariableContainer(constants_1.KEY_VALUE_PAIR_KEY, '', mapKeyType);
                    if (keyIsRef) {
                        const keyRef = this.logContext
                            .getRefsMap()
                            .get(extentValueEntry.keyDisplayValue);
                        if (keyRef &&
                            keyRef.type.toLowerCase() === constants_1.LC_APEX_PRIMITIVE_STRING) {
                            keyIsRef = false;
                    ...
                    const valContainer = new apexReplayDebug_1.ApexVariableContainer(constants_1.KEY_VALUE_PAIR_VALUE, '', valueCollectionType);
                    if (valueIsRef) {
                        const valueRef = this.logContext
                            .getRefsMap()
                            .get(extentValueEntry.value.value);
                        if (valueRef &&
                            valueRef.type.toLowerCase() === constants_1.LC_APEX_PRIMITIVE_STRING) {
                            valueIsRef = false;
                ...
                for (let i = 0; i < values.length; i++) {
                    ...
                    if (this.isAddress(values[i].value)) {
                        ...
                        if (valRef &&
                            valRef.type.toLowerCase() === constants_1.LC_APEX_PRIMITIVE_STRING) {
                            valString = valRef.value;
                        }
                        else {
                            hasInnerRefs = true;
                        }
                        refContainer.variables.set(i.toString(), new apexReplayDebug_1.ApexVariableContainer(i.toString(), valString, valueCollectionType, values[i].value));
                    }
                    else {
                        refContainer.variables.set(i.toString(), new apexReplayDebug_1.ApexVariableContainer(i.toString(), this.createStringFromExtentValue(values[i].value), valueCollectionType));
                    }
                }
            }
            ...
            for (const extentValueEntry of extentValue.value.entry) {
                ...
                if (valueIsRef) {
                    ...
                    refContainer.variables.set(extentValueEntry.keyDisplayValue, new apexReplayDebug_1.ApexVariableContainer(extentValueEntry.keyDisplayValue, valString, '', extentValueEntry.value.value));
                }
                else {
                    refContainer.variables.set(extentValueEntry.keyDisplayValue, new apexReplayDebug_1.ApexVariableContainer(extentValueEntry.keyDisplayValue, this.createStringFromExtentValue(extentValueEntry.value.value), ''));
                }
            }
        }
        // If the reference doesn't contain any child references then it's value can set here.
        if (!hasInnerRefs) {
            refContainer.value = this.createStringFromVarContainer(refContainer, new Set());
        }
    }
    createStringFromExtentValue(value) {
    ...
    createVariableFromReference(varName, refVariable, visitedMap, updateAfterVarCreation) {
        ...
        try {
            ...
            for (const entry of Array.from(namedVarContainer.variables.entries())) {
                ...
                // 'value'
                if (childVarContainer.type === constants_1.KEY_VALUE_PAIR) {
                    // process the key
                    const keyVarContainer = childVarContainer.variables.get(constants_1.KEY_VALUE_PAIR_KEY);
                    ...
                    if (keyVarContainer && keyVarContainer.ref) {
                        ...
                        if (keyRef) {
                            const updatedKeyVarContainer = this.createVariableFromReference(constants_1.KEY_VALUE_PAIR_KEY, keyRef, visitedMap, updateAfterVarCreation);
                            if (updatedKeyVarContainer) {
                                keyName = updatedKeyVarContainer.value;
                                childVarContainer.variables.set(constants_1.KEY_VALUE_PAIR_KEY, updatedKeyVarContainer);
                            }
                    ...
                    // process the value
                    const valueVarContainer = childVarContainer.variables.get(constants_1.KEY_VALUE_PAIR_VALUE);
                    ...
                    if (valueVarContainer && valueVarContainer.ref) {
                        ...
                        if (valueRef) {
                            const updatedValueVarContainer = this.createVariableFromReference(constants_1.KEY_VALUE_PAIR_VALUE, valueRef, visitedMap, updateAfterVarCreation);
                            if (updatedValueVarContainer) {
                                valueVal = updatedValueVarContainer.value;
                                childVarContainer.variables.set(constants_1.KEY_VALUE_PAIR_VALUE, updatedValueVarContainer);
                            }
                            // The value happened to match our pattern for an address but isn't in the references list.
                            // Set the value to the ref value and clear the ref.
                        }
                    ...
                    if (childVarRefContainer) {
                        const updatedChildContainer = this.createVariableFromReference(childVarName, childVarRefContainer, visitedMap, updateAfterVarCreation);
                ...
                namedVarContainer.value = this.createStringFromVarContainer(namedVarContainer, new Set());
            }
            ...
            if (visitedMap.size === 0) {
                updateAfterVarCreation.forEach(element => {
                    ...
                    varContainer.value = this.createStringFromVarContainer(varContainer, new Set());
                });
            }
        }
    }
    ...
    getKeyTypeForMap(typeName, collectionType) {
...
//# sourceMappingURL=heapDumpService.js.map