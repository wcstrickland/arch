...
exports.messages = {
    ...
    up_to_five_checkpoints: '最大 5 つのうち、%s つのチェックポイントを設定しています。5 つ以下になるようチェックポイントを削除または無効化してください。',
    checkpoints_can_only_be_on_valid_apex_source: 'チェックポイントApex ソースの有効な行に設定できます。無効なチェックポイントの位置 : URI=%s, 行=%s',
    local_source_is_out_of_sync_with_the_server: 'ローカルソースはサーバーと同期していません。ローカルで行った変更を組織にプッシュし、組織で行った変更をローカルプロジェクトにプルしてください。',
    ...
    checkpoint_creation_status_processing_complete_success: 'ステップ 6/6: チェックポイントが正常に作成されたことを確認しています。',
    checkpoint_upload_in_progress: '組織へのチェックポイントの更新が進行中です。チェックポイントを新たに追加、削除する前に更新の完了をお待ちください。',
    checkpoint_upload_error_wrap_up_message: 'チェックポイントにはエラーがあります。出力されるエラーを修正し、%s を再度実行してください。',
    ...
    unable_to_retrieve_active_user_for_sfdx_project: 'SFDX プロジェクトの有効なユーザを取得できませんでした。',
    ...
    no_line_breakpoint_information_for_current_project: '現在のプロジェクトにはブレークポイントの情報を含む行がありません。',
    line_breakpoint_information_success: '言語サーバからブレークポイントの行を取得しました。',
    language_client_not_ready: '言語サーバからブレークポイントの情報を取得できませんでした。言語サーバが起動していません。',
