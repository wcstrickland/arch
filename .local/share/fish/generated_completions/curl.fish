...
complete -c curl -l doh-cert-status -d '(all) Same as --cert-status but used for DoH (DNS-over-HTTPS).   Example: '
complete -c curl -l doh-insecure -d '(all) Same as -k, --insecure but used for DoH (DNS-over-HTTPS).   Example: '
complete -c curl -l doh-url -d '(all) Specifies which DNS-over-HTTPS (DoH) server to use to resolve hostnames…'
...
complete -c curl -s k -l insecure -d '(TLS) By default, every SSL connection curl makes is verified to be secure'
...
complete -c curl -l mail-rcpt -d '(SMTP) Specify a single e-mail address, user name or mailing list name'
complete -c curl -s M -l manual -d 'Manual.  Display the huge help text.   Example: . nf  curl --manual . fi'
